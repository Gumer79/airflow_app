...
# -*- coding: UTF-8 -*-
"""
DAG для автоматической загрузки данных о заказах Wildberries в GCS и BigQuery.

Использование:
    - DAG запускается автоматически каждый день в 02:30 UTC
...
Функционал:
    ...
    2. Для каждой компании:
       - Запрашивает данные о заказах через Wildberries API
...
Структура данных в GCS:
    - Путь: gs://app_s3/wildberries/orders/{date}/{company_id}.json
    - Формат: JSON массив с данными о заказах

Таблица BigQuery:
    - Проект: shirman-group-app
    - Dataset: wildberries_raw
    - Таблица: orders_raw (партиционирована по полю date)

Поля данных:
    Данные о заказе аналогичны данным о продажах, но включают все заказы
    (включая те, что еще не завершены). Для определения заказа рекомендуется
    использовать поле srid.
...
Расписание: Каждый день в 02:30 UTC
"""
...
def fetch_and_save_orders(**kwargs):
    """Получает данные о заказах для всех компаний и сохраняет их в GCS."""
    ti = kwargs["ti"]
    ...
    logging.info("=" * 80)
    logging.info(f"🛒 НАЧАЛО ОБРАБОТКИ ЗАКАЗОВ ЗА ДАТУ: {target_date.isoformat()}")
    ...
    for idx, company in enumerate(companies, 1):
        ...
        try:
            ...
            orders_data = wb_api.get_orders(date_from=target_date, flag=1)
            
            logging.info(f"✅ Получено записей: {len(orders_data)}")

            if not orders_data:
                logging.info(f"ℹ️  Нет данных о заказах за {target_date}")
                stats["empty"] += 1
                continue

            file_name = f"wildberries/orders/{target_date.isoformat()}/{company_id}.json"

            logging.info(f"💾 Загрузка в GCS: gs://{GCS_BUCKET}/{file_name}")
            gcs_hook.upload(
                bucket_name=GCS_BUCKET,
                object_name=file_name,
                data=json.dumps(orders_data, indent=4, ensure_ascii=False),
    ...
    logging.info(f"📭 Без данных о заказах: {stats['empty']} компаний")
...
def load_gcs_to_bigquery(**kwargs):
    ...
    try:
        gcs_hook = GCSHook(gcp_conn_id=GCP_CONN_ID)
        prefix = f"wildberries/orders/{date_prefix}/"
        ...
        # Схема таблицы для данных о заказах
        schema = [
            ...
            bq_client.SchemaField("spp", "NUMERIC", mode="NULLABLE"),
            bq_client.SchemaField("finishedPrice", "NUMERIC", mode="NULLABLE"),
            bq_client.SchemaField("priceWithDisc", "NUMERIC", mode="NULLABLE"),
            bq_client.SchemaField("isCancel", "BOOLEAN", mode="NULLABLE"),
            bq_client.SchemaField("cancelDate", "TIMESTAMP", mode="NULLABLE"),
        ...
        table_id = f"{BIGQUERY_PROJECT}.{BIGQUERY_WILDBERRIES_DATASET}.{BIGQUERY_ORDERS_TABLE}"
        ...
        try:
            ...
            logging.info(f"✅ Таблица {BIGQUERY_ORDERS_TABLE} уже существует")
        except NotFound:
            client.create_table(table_ref, exists_ok=True)
            logging.info(f"🆕 Таблица {BIGQUERY_ORDERS_TABLE} создана")
        ...
        for idx, file_path in enumerate(files, 1):
            try:
                ...
                for order in orders_data:
                    ...
                    order_record["company_id"] = company_id
                    order_record["data_ingestion_time"] = data_ingestion_time.isoformat() + "Z"
                    all_rows.append(order_record)

                logging.info(f"     ✅ Записей: {len(orders_data)}, Компания: {company_id}")
                processed_files += 1
...
with DAG(
    ...
    schedule=SCHEDULE_ORDERS,  # Ежедневно в 02:30 UTC
    catchup=False,
    tags=["wildberries", "orders", "gcs", "bigquery"],
    doc_md=__doc__,
) as dag:
    ...
    task_fetch_and_save = PythonOperator(
        task_id="fetch_and_save_orders",
        python_callable=fetch_and_save_orders,
    )
