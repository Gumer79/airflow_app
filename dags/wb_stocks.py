...
# -*- coding: UTF-8 -*-
"""
DAG для автоматической загрузки данных об остатках Wildberries в GCS и BigQuery.

Использование:
    - DAG запускается автоматически каждый день в 04:00 UTC
...
Функционал:
    ...
    2. Для каждой компании:
       - Запрашивает данные об остатках через Wildberries API
...
Структура данных в GCS:
    - Путь: gs://app_s3/wildberries/stocks/{date}/{company_id}.json
    - Формат: JSON массив с данными об остатках

Таблица BigQuery:
    - Проект: shirman-group-app
    - Dataset: wildberries_raw
    - Таблица: stocks_raw (партиционирована по полю lastChangeDate)
...
Поля данных:
    ...
    - nmId: Артикул WB
    - barcode: Штрихкод
    - quantity: Количество полное (на складе и в пути)
...
def fetch_and_save_stocks(**kwargs):
    """Получает данные об остатках для всех компаний и сохраняет их в GCS."""
    ti = kwargs["ti"]
    ...
    # Для остатков используем текущую дату
    ...
    logging.info("=" * 80)
    logging.info(f"📦 НАЧАЛО ОБРАБОТКИ ОСТАТКОВ ЗА ДАТУ: {target_date.isoformat()}")
    ...
    for idx, company in enumerate(companies, 1):
        ...
        try:
            ...
            # Для полного остатка используем раннюю дату
            stocks_data = wb_api.get_stocks(date_from="2019-06-20")
            
            logging.info(f"✅ Получено записей: {len(stocks_data)}")

            if not stocks_data:
                logging.info(f"ℹ️  Нет данных об остатках")
                stats["empty"] += 1
                continue

            file_name = f"wildberries/stocks/{target_date.isoformat()}/{company_id}.json"

            logging.info(f"💾 Загрузка в GCS: gs://{GCS_BUCKET}/{file_name}")
            gcs_hook.upload(
                bucket_name=GCS_BUCKET,
                object_name=file_name,
                data=json.dumps(stocks_data, indent=4, ensure_ascii=False),
    ...
    logging.info(f"📭 Без данных об остатках: {stats['empty']} компаний")
...
def load_gcs_to_bigquery(**kwargs):
    ...
    try:
        gcs_hook = GCSHook(gcp_conn_id=GCP_CONN_ID)
        prefix = f"wildberries/stocks/{date_prefix}/"
        ...
        # Схема таблицы для данных об остатках
        schema = [
            ...
            bq_client.SchemaField("inWayToClient", "INTEGER", mode="NULLABLE"),
            bq_client.SchemaField("inWayFromClient", "INTEGER", mode="NULLABLE"),
            bq_client.SchemaField("quantityFull", "INTEGER", mode="NULLABLE"),
            ...
            bq_client.SchemaField("Price", "NUMERIC", mode="NULLABLE"),
            bq_client.SchemaField("Discount", "NUMERIC", mode="NULLABLE"),
            bq_client.SchemaField("isSupply", "BOOLEAN", mode="NULLABLE"),
            bq_client.SchemaField("isRealization", "BOOLEAN", mode="NULLABLE"),
            bq_client.SchemaField("SCCode", "STRING", mode="NULLABLE"),
            bq_client.SchemaField("company_id", "STRING", mode="REQUIRED"),
            bq_client.SchemaField("data_ingestion_time", "TIMESTAMP", mode="REQUIRED"),
        ]

        table_id = f"{BIGQUERY_PROJECT}.{BIGQUERY_WILDBERRIES_DATASET}.{BIGQUERY_STOCKS_TABLE}"
        table_ref = bq_client.Table(table_id, schema=schema)
        table_ref.time_partitioning = bq_client.TimePartitioning(
            type_=bq_client.TimePartitioningType.DAY, field="lastChangeDate"
        )

        try:
            client.get_table(table_id)
            logging.info(f"✅ Таблица {BIGQUERY_STOCKS_TABLE} уже существует")
        except NotFound:
            client.create_table(table_ref, exists_ok=True)
            logging.info(f"🆕 Таблица {BIGQUERY_STOCKS_TABLE} создана")
        ...
        for idx, file_path in enumerate(files, 1):
            try:
                ...
                stocks_data = json.loads(file_content)
                ...
                for stock in stocks_data:
                    stock_record = stock.copy()
                    stock_record["company_id"] = company_id
                    stock_record["data_ingestion_time"] = data_ingestion_time.isoformat() + "Z"
                    all_rows.append(stock_record)

                logging.info(f"     ✅ Записей: {len(stocks_data)}, Компания: {company_id}")
                processed_files += 1
...
with DAG(
    ...
    schedule=SCHEDULE_STOCKS,  # Ежедневно в 04:00 UTC
    catchup=False,
    tags=["wildberries", "stocks", "gcs", "bigquery"],
    doc_md=__doc__,
) as dag:
